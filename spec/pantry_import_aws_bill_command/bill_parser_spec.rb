...
RSpec.describe Wonga::BillParser do
  context 'parse bill' do
    let(:result) { subject.parse IO.read('spec/fixtures/bill.csv') }
    ...
    it 'parses a non estimated instance' do
      ...
      expect(result[:ec2_total][0][:estimated]).to be false
    end
    ...
    it 'parses the estimated instance' do
      ...
      expect(result[:ec2_total][1][:estimated]).to be true
    end
  end
end
